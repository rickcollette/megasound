...
func multiplyChannels(left, right float64, s megasound.Streamer) megasound.Streamer {
...
type movingStreamer struct {
	x, y         float64
	velX, velY   float64
	leftDoppler  megasound.Streamer
	rightDoppler megasound.Streamer
}

func newMovingStreamer(sr megasound.SampleRate, x, y float64, streamer megasound.Streamer) *movingStreamer {
	...
	leftEar, rightEar := megasound.Dup(streamer)
...
func main() {
	...
	leftCh, rightCh := megasound.Dup(streamer)
