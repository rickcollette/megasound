...
// NewEqualizer returns a megasound.Streamer that modifies the stream based on the EqualizerSection slice that is passed in.
// The SampleRate (sr) must match that of the Streamer.
func NewEqualizer(st megasound.Streamer, sr megasound.SampleRate, s EqualizerSections) megasound.Streamer {
	return &equalizer{
		streamer: st,
