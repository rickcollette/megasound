// Package speaker implements playback of megasound.Streamer values through physical speakers.
package speaker
...
func Init(sampleRate megasound.SampleRate, bufferSize int) error {
	mu.Lock()
	defer mu.Unlock()
...
func Play(s ...megasound.Streamer) {
	mu.Lock()
	mixer.Add(s...)
	mu.Unlock()
}
