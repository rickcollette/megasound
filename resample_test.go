...
func TestResample(t *testing.T) {
	for _, numSamples := range []int{8, 100, 500, 1000, 50000} {
		for _, old := range []megasound.SampleRate{100, 2000, 44100, 48000} {
			for _, new := range []megasound.SampleRate{100, 2000, 44100, 48000} {
				...
				got := collect(megasound.Resample(3, old, new, s))
...
func resampleCorrect(quality int, old, new megasound.SampleRate, p [][2]float64) [][2]float64 {
