...
func TestFormatEncodeDecode(t *testing.T) {
	formats := make(chan megasound.Format)
	go func() {
		defer close(formats)
		for _, sampleRate := range []megasound.SampleRate{100, 2347, 44100, 48000} {
			for _, numChannels := range []int{1, 2, 3, 4} {
				for _, precision := range []int{1, 2, 3, 4, 5, 6} {
					formats <- megasound.Format{
...
func TestBufferAppendPop(t *testing.T) {
	...
	for format := range formats {
		b := megasound.NewBuffer(format)
		b.Append(megasound.Silence(768))
		if b.Len() != 768 {
