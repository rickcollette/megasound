...
func SawtoothTone(sr megasound.SampleRate, freq float64) (megasound.Streamer, error) {
	dt := freq / float64(sr)
...
// use other wrappers of this package to change amplitude or add time limit.
// sampleRate must be at least two times grater then frequency, otherwise this function will return an error.
func SawtoothToneReversed(sr megasound.SampleRate, freq float64) (megasound.Streamer, error) {
	dt := freq / float64(sr)
