...
func SquareTone(sr megasound.SampleRate, freq float64) (megasound.Streamer, error) {
	dt := freq / float64(sr)

	if dt >= 1.0/2.0 {
		return nil, pkgerrors.New("megasound square tone generator: samplerate must be at least 2 times grater then frequency")
	}
