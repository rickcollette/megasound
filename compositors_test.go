...
func randomDataStreamer(numSamples int) (s megasound.StreamSeeker, data [][2]float64) {
...
func collect(s megasound.Streamer) [][2]float64 {
	var (
		result [][2]float64
		buf    [479][2]float64
	)
...
func TestTake(t *testing.T) {
	for i := 0; i < 7; i++ {
		...
		want := data[:take]
		got := collect(megasound.Take(take, s))
...
func TestLoop(t *testing.T) {
	for i := 0; i < 7; i++ {
		for n := 0; n < 5; n++ {
			...
			got := collect(megasound.Loop(n, s))
